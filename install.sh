...
[Unit]
Description=3xui-Aggregator traffic redirect helper (${INSTANCE_NAME})
...
[Unit]
Description=3xui-Aggregator web updater (${INSTANCE_NAME})
After=network-online.target docker.service
Wants=network-online.target
...
settings_transfer_cli() {
  ...
  case "$action" in
    export)
      shift || true
      local output_path="${1:-/root/aggregator-settings-$(date +%F-%H%M%S).nxsettings}"
