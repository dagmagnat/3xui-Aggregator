(function () {
  ...
  async function runImport(dryRun) {
    ...
    if (nodeMode === 'selected' && nodeIds.length === 0) {
      setStatus('Для выбранного режима отметь хотя бы один узел этой панели.', 'error');
      return;
    }
    ...
    if (!dryRun && !window.confirm('Импортировать клиентов в локальную базу Aggregator? Перед записью будет создана резервная копия.')) return;
